...
class Admin extends DatabaseConnection
{
    ...
    public function createUsers($_conn)
    {
        ...
        if (move_uploaded_file($temp, $uploadedPath)) {
            ...
            $sql = "INSERT INTO `member`( `username`, `password`,`first_name`, `gender`, `birthdate`, `city`, `state`, `picture`) VALUES ('$username','$password','$name','$gender','$bod','$city','$state','$newName');";
            if (!$_conn) {
                $response = array("error" => "there is an error connection", "status" => false);
            } else {
                $result = $_conn->query($sql);
    ...
    public function updateUsers($conn)
    {
        ...
        if ($hasProfile == "true") {
            ...
            $uploadedPath = "../uploads/" . $newName;
            if (move_uploaded_file($temp, $uploadedPath)) {
                $sql = "UPDATE `member` SET `username`='$username',`password`='$password',`first_name`='$name',`gender`='$gender',`birthdate`='$bod',`city`='$city',`state`='$state',`picture`='$newName' WHERE `user_id`='$id';";
                if (!$conn) {
                    $response = array("error" => "there is an error connection", "status" => false);
                } else {
                    $result = $conn->query($sql);
            ...
            $sql = "UPDATE `member` SET `username`='$username',`password`='$password',`first_name`='$name',`gender`='$gender',`birthdate`='$bod',`city`='$city',`state`='$state' WHERE `user_id`='$id';";
            if (!$conn) {
                $response = array("error" => "there is an error connection", "status" => false);
            } else {
                $result = $conn->query($sql);
